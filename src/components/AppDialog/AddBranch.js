...
class AddBranch extends Component {
  ...
  onFormSubmit(event) {
    ...
    let options = {
      afterSuccess: () => {
        _this.props.actions.openSnackbar('Branch created')
      ...
      afterError: () => {
        _this.props.actions.openSnackbar('Failed to create Branch');
      }
    };
  ...
  render() {
    ...
    return (
      <div style={STYLES.ADD_APP_FORM.CONTAINER}>
        <h1 style={STYLES.ADD_APP_FORM.HEADER}>Add Branch</h1>
        <div style={STYLES.ADD_APP_FORM.WRAP}>
          <FormControl error={BranchId.dirty && BranchId.error} style={{ width: '100%' }}>
          ...
          <Button onClick={this.onFormSubmit} variant="raised" color="primary" style={STYLES.ADD_APP_FORM.BUTTON}>Create Branch</Button>
        </div>
      </div>
    );
  }
